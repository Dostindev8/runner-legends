...
const PALETTES: Record<WorldId, Palette> = {
  neon: { sky: '#050218', fog: 0.008, accent: 0xff3cb4, ground: 0x0a1830, sun: 0xff66cc },
  golden: { sky: '#120804', fog: 0.01, accent: 0xffd24a, ground: 0x1a1208, sun: 0xffaa44 },
  ice: { sky: '#040c18', fog: 0.008, accent: 0x9ff0ff, ground: 0x0a2038, sun: 0xaaddff },
  coliseum: { sky: '#100808', fog: 0.008, accent: 0xffb060, ground: 0x1a1010, sun: 0xffcc88 },
  abyssal: { sky: '#020818', fog: 0.012, accent: 0x2ee8c0, ground: 0x021418, sun: 0x44ffcc },
  celestial: { sky: '#0c1430', fog: 0.007, accent: 0xa8c8ff, ground: 0x101830, sun: 0xcce0ff },
  quantum: { sky: '#040c08', fog: 0.009, accent: 0x4aff8a, ground: 0x081810, sun: 0x66ff99 },
  igneous: { sky: '#180404', fog: 0.01, accent: 0xff6030, ground: 0x1a0804, sun: 0xff4422 },
  fractal: { sky: '#080810', fog: 0.008, accent: 0xc850ff, ground: 0x0c0c18, sun: 0xaa66ff },
  final: { sky: '#050018', fog: 0.009, accent: 0xffd24a, ground: 0x0a0018, sun: 0xffee88 },
};
...
export async function createWorld(id: WorldId, quality: QualityPreset): Promise<WorldHandle> {
  ...
  const sun = new THREE.DirectionalLight(pal.sun, 1.8);
  ...
  const hemi = new THREE.HemisphereLight(pal.accent, pal.ground, 0.7);
  root.add(hemi);

  // Fill light toward camera so character never silhouettes to black
  ...
  for (let i = 0; i < count; i++) {
    ...
    const z = (i * (200 / count)); // denser near spawn so menu shows city
    const h = 2 + (i % 5) * 1.4;
